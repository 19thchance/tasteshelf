...
type LoopingTickerBaseProps<T> = {
  containerClassName: string;
  copyTrackClassName: string;
...
export function LoopingTickerBase<T>({
  containerClassName,
  copyTrackClassName,
  emptyState,
  getItemKey,
  ...
  useLayoutEffect(() => {
    ...
    if (!container || !baseTrack || !items.length) {
      return;
    }
  ...
  useAnimationFrame((timestamp) => {
    ...
    if (speedTransition) {
      ...
      const progress = Math.min(
        1,
        (timestamp - speedTransition.startTimestamp) /
          AUTO_SCROLL_TRANSITION_MS,
      );

      autoScrollSpeedRef.current =
        speedTransition.from +
        (speedTransition.to - speedTransition.from) * easeOut(progress);
        ...
        {buildCopyIndexes(repeatCount).map((copyIndex) => (
          ...
          >
            {items.map((item, itemIndex) => {
              ...
              return (
                <Fragment key={`${copyIndex}-${itemKey}`}>
